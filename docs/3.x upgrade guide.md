...
### Removed Form `autocomplete` prop

In v2.x the autocomplete prop was supported via both the `autocomplete` prop and the `autoComplete` prop. The `autocomplete` prop was marked as deprecated, and warned about this behavior. 

In v3.x the `autocomplete` prop has been removed in favor of just the `autoComplete` prop, and any code previously using `autocomplete` should migrate to `autoComplete`